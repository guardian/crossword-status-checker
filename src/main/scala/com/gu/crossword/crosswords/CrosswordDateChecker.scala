...
object CrosswordDateChecker extends APIChecker {
  ...
  def getAllCrosswordStatusesForDate(date: LocalDate)(config: Config): Future[List[CrosswordReadyStatus]] = {
    ...
    val crosswordStatuses = CrosswordTypeHelpers.allTypes.flatMap { cw =>
      cw.getNo(date).map { no =>
        ...
        val ready = checkIfCrosswordInApis(path)(config).map(r => CrosswordReadyStatus(cw.name, no, r.inCapiPreview, date))
        ready
      }
    }
  ...
  def alertForBadCrosswords(statuses: List[CrosswordReadyStatus])(config: Config) = {

    val invalidXWords = statuses.filter(_.ready == false)
    invalidXWords.foreach(c => {
      println(s"Found invalid crossword - ${c.crosswordType} crossword ${c.number} for ${c.date}")
      val alertString = s"Warning - ${c.crosswordType} crossword ${c.number} for ${c.date} not ready! To view detailed status, see " +
